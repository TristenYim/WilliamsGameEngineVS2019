...
void DefenseBot::update(sf::Time& elapsed) {
	float distance = DRIVING_SPEED * elapsed.asMilliseconds();
	sf::Vector2f neoPosition = sprite_.getPosition();

	sf::Vector2f positionToReach = neoPosition;

	sf::Vector2i positionToCheck = PlayingField::findRelativePosition(neoPosition);
	...
	positionToReach.x += xMovementIncrement * DRIVING_SPEED * elapsed.asMilliseconds();
	positionToReach.y += yMovementIncrement * DRIVING_SPEED * elapsed.asMilliseconds();

	while (true) {
		if (!PlayingField::canThisObjectBeAt(sf::Vector2i(positionToCheck.x + xCheckIncrement, positionToCheck.y + yCheckIncrement), DEFENSE_TAG)) {
			neoPosition = PlayingField::findAbsolutePosition(sf::Vector2i(positionToCheck.x + xCheckIncrement, positionToCheck.y - yCheckIncrement));
			break;
		}
		if (PlayingField::findRelativePosition(positionToReach) == positionToCheck) {
